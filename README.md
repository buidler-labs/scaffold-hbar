...
1. **Node.js** v20.x LTS (recommended: 20.18.3 or higher). Node 22+ may work but is not officially tested.
...
This template’s Subscription Guide recipe is gate **0–1** (static + yarn). If you deepen the recipe with Playwright (gate 2) or on-chain validation (gate 3.5), install the optional peers at the **project root** with Yarn (do not use `npm install` in this repo):
...
Also: **`.harness/`** — tracked harness recipe (`spec.yaml`, `prd.md`, `validators/`). Run `yarn harness:extend` to add the Subscription Guide page (`/guide`) in place.
...
If you see `Access to fetch at 'https://testnet.hashio.io/api' has been blocked by CORS policy`, the default RPC endpoint doesn't support browser CORS requests. 

**Solutions:**
1. **Use a CORS-friendly RPC** - Set `NEXT_PUBLIC_HEDERA_TESTNET_RPC_URL` in `packages/nextjs/.env` to a CORS-enabled endpoint (e.g., [Arkhia](https://arkhia.io/) or your own proxy)
2. **Use wallet-connected operations** - Most wallet operations work fine since wallets handle RPC internally

### Wallet address is EVM format, not Hedera account ID

The deployer wallet generated by `yarn hardhat:account:generate` shows an EVM address (0x...), not a Hedera account ID (0.0.xxxxx). Both formats work with the [Hedera Portal Faucet](https://portal.hedera.com/faucet) — you can paste either format to fund your account.
