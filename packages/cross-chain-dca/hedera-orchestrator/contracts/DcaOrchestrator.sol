...
contract DcaOrchestrator {
    ...
    function executeDca(uint256 planId) external {
        ...
        require(
            block.timestamp >= plan.lastExecutionTime + plan.intervalSeconds,
            "DcaOrchestrator: too soon"
        );
    ...
    function _dispatchViaBridge(uint256 planId) internal {
        DcaPlan storage plan = plans[planId];
        bridgeSender.send{value: plan.feeForSender}(planId, plan.amountPerExecution, plan.targetToken, plan.minAmountOut);
    }
