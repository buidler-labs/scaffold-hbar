...
async function deployAll() {
  ...
  const gateway = (await (
    await ethers.getContractFactory("MockAxelarGateway")
  ).deploy()) as MockAxelarGateway;

  const gasService = (await (
    await ethers.getContractFactory("MockAxelarGasService")
  ).deploy()) as MockAxelarGasService;
...
describe("AxelarMessageSender — access control", function () {
  ...
  it("send() reverts for non-authorized caller", async function () {
    const { sender, other } = await deployAll();
    await expect(
      sender.connect(other).send(PLAN_ID, AMOUNT, TARGET_TOKEN, MIN_AMOUNT_OUT)
    ).to.be.revertedWith("AxelarMessageSender: not authorized");
  });
});
...
describe("AxelarMessageSender — send()", function () {
  it("calls gas service with correct value and routing", async function () {
    ...
    await sender
      .connect(caller)
      .send(PLAN_ID, AMOUNT, TARGET_TOKEN, MIN_AMOUNT_OUT, { value: AXELAR_GAS_FEE });
  ...
  it("reverts when called with zero msg.value", async function () {
    ...
    await expect(
      sender.connect(caller).send(PLAN_ID, AMOUNT, TARGET_TOKEN, MIN_AMOUNT_OUT)
    ).to.be.revertedWith("AxelarMessageSender: zero value");
  });
});
...
describe("AxelarMessageSender — withdraw()", function () {
  ...
  it("reverts when called by non-owner", async function () {
    const { sender, other } = await deployAll();
    await expect(sender.connect(other).withdraw()).to.be.revertedWith(
      "AxelarMessageSender: not owner"
    );
  });
