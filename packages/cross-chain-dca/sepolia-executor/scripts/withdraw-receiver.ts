...
async function main() {
  ...
  const ethBalance = await provider.getBalance(receiverAddr);
  console.log("\n  ETH balance: ", ethers.formatEther(ethBalance), "ETH");
  if (ethBalance > 0n) {
    const tx = await receiver.withdrawETH();
    await tx.wait();
  ...
  const tokens = await getTokensWithBalance(receiverAddr, provider);
