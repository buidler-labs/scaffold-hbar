...
function main() {
  ...
  for (const [chainId, chainConfig] of Object.entries(allGeneratedContracts)) {
    ...
    if (!outputContracts[outputChainId]) outputContracts[outputChainId] = {};
    Object.assign(outputContracts[outputChainId], chainConfig);
  }

  // Generate the deployedContracts content
  const fileContent = Object.entries(outputContracts).reduce((content, [chainId, chainConfig]) => {
    ...
    return `${content}${parseInt(chainId).toFixed(0)}:${JSON.stringify(cleanedChainConfig, null, 2)},`;
  }, "");
  ...
  console.log(`📝 Updated TypeScript contract definition file on ${join(NEXTJS_TARGET_DIR, "deployedContracts.ts")}`);
}
