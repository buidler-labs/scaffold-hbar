...
contract MemejobDCAStrategyTest is Test {
    ...
    function test_planBuyRevertsOnSlippage() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MemejobDCAStrategy.MemejobDCAStrategy__SlippageExceeded.selector, expectedCost, tooLowMax
            )
        );
    ...
    function test_planBuyRevertsOnInsufficientVaultBalance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MemejobDCAStrategy.MemejobDCAStrategy__InsufficientVaultBalance.selector, ONE_TOKEN * PRICE_PER_UNIT, 0
            )
        );
        strategy.plan(config);
    }
    ...
    function test_planSellRevertsOnInsufficientTokens() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MemejobDCAStrategy.MemejobDCAStrategy__InsufficientVaultBalance.selector, ONE_TOKEN, 0
            )
        );
        strategy.plan(config);
    }
