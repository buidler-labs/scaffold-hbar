...
async function main() {
  ...
  if (planId === undefined) throw new Error("PlanCreated event not found in receipt — check the tx on HashScan");
  ...
  for (const log of receipt!.logs) {
    try {
      ...
      if (parsed?.name === "ScheduleAttempted") {
        ...
        if (!ok) console.warn("  ⚠️  Scheduling failed — first execution will need to be triggered manually.");
        break;
      }
    } catch {
      /* not our event */
