...
async function deployAll() {
  ...
  const mockBridgeSender = (await (await ethers.getContractFactory("MockBridgeSender")).deploy()) as MockBridgeSender;
...
describe("DcaOrchestrator — createPlan / cancelPlan", function () {
  ...
  it("emits PlanCreated with correct arguments", async function () {
    const { harness, owner } = await deployAll();
    await expect(harness.createPlan(AMOUNT, FEE_FOR_SENDER, INTERVAL, TARGET_TOKEN, MIN_AMOUNT_OUT, 5))
  ...
  it("reverts when amountPerExecution is 0", async function () {
    const { harness } = await deployAll();
    await expect(harness.createPlan(0, FEE_FOR_SENDER, INTERVAL, TARGET_TOKEN, MIN_AMOUNT_OUT, 5)).to.be.revertedWith(
      "DcaOrchestrator: amount must be > 0",
    );
  });

  it("reverts when intervalSeconds is 0", async function () {
    const { harness } = await deployAll();
    await expect(harness.createPlan(AMOUNT, FEE_FOR_SENDER, 0, TARGET_TOKEN, MIN_AMOUNT_OUT, 5)).to.be.revertedWith(
      "DcaOrchestrator: interval must be > 0",
    );
  });
  ...
  it("cancelPlan sets active to false and emits PlanCancelled", async function () {
    const { harness, owner } = await deployAll();
    await harness.createPlan(AMOUNT, FEE_FOR_SENDER, INTERVAL, TARGET_TOKEN, MIN_AMOUNT_OUT, 5);
    await expect(harness.cancelPlan(0)).to.emit(harness, "PlanCancelled").withArgs(0n, owner.address);
    expect((await harness.plans(0)).active).to.equal(false);
  });

  it("cancelPlan reverts when called by non-owner", async function () {
    const { harness, other } = await deployAll();
    await harness.createPlan(AMOUNT, FEE_FOR_SENDER, INTERVAL, TARGET_TOKEN, MIN_AMOUNT_OUT, 5);
    await expect(harness.connect(other).cancelPlan(0)).to.be.revertedWith("DcaOrchestrator: not plan owner");
  });
...
describe("DcaOrchestrator — owner / withdraw()", function () {
  ...
  it("reverts when called by non-owner", async function () {
    const { harness, other } = await deployAll();
    await expect(harness.connect(other).withdraw()).to.be.revertedWith("DcaOrchestrator: not owner");
  });
...
describe("DcaOrchestrator — executeDca", function () {
  ...
  it("deactivates plan and emits PlanCancelled when maxExecutions reached", async function () {
    ...
    await advanceInterval();
    await expect(harness.executeDca(0)).to.emit(harness, "PlanCancelled").withArgs(0n, owner.address);
