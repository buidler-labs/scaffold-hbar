...
async function deployAll() {
  ...
  const router = (await (await ethers.getContractFactory("MockSwapRouter")).deploy()) as MockSwapRouter;

  const usdc = (await (await ethers.getContractFactory("MockERC20")).deploy("USD Coin", "USDC", 6)) as MockERC20;

  const weth = (await (await ethers.getContractFactory("MockERC20")).deploy("Wrapped Ether", "WETH", 18)) as MockERC20;
...
describe("DcaExecutor — access control", function () {
  ...
  it("setAuthorizedCaller reverts for zero address", async function () {
    ...
    await expect(executor.setAuthorizedCaller(ethers.ZeroAddress)).to.be.revertedWith("DcaExecutor: zero address");
  });
...
describe("DcaExecutor — handleDcaExecution swap", function () {
  ...
  it("emits SwapExecuted with correct values", async function () {
    ...
    await expect(executor.connect(caller).handleDcaExecution(7n, AMOUNT_IN, await weth.getAddress(), 100))
...
describe("DcaExecutor — withdrawToken()", function () {
  ...
  it("reverts for zero token address", async function () {
    const { executor } = await deployAll();
    await expect(executor.withdrawToken(ethers.ZeroAddress)).to.be.revertedWith("DcaExecutor: zero token address");
  });
