...
import {
  HederaMirrorNetwork,
  TokenIdResolutionStatus,
  resolveTokenIdFromMirrorTransaction,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const status: TokenIdResolutionStatus = await resolveTokenIdFromMirrorTransaction({
      mirrorBaseUrl: base,
      transactionId: txId,
      fetchFn: fetch,
    });
    ...
    console.error("[api/hedera/token-id-by-transaction]", error);
