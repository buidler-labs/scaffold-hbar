...
import { CalendarIcon, CurrencyDollarIcon, TagIcon } from "@heroicons/react/24/outline";
import { formatDate, tinybarsToHbar } from "~~/utils/hedera";

interface PendingSaleListing {
  serialNumber: string;
  isAuction: boolean;
  price: string;
...
interface PendingSalesListingCardProps {
  listing: PendingSaleListing;
}

export const PendingSalesListingCard = ({ listing }: PendingSalesListingCardProps) => {
  const { serialNumber, isAuction, price, effectiveStartDate } = listing;
  ...
  return (
    <div className="card bg-base-100 border border-primary/50 shadow-md rounded-xl animate-pulse">
      <div className="card-body p-4">
        ...
        <span className={`badge ${isAuction ? "badge-secondary" : "badge-primary"} badge-sm`}>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span>
              {tinybarsToHbar(BigInt(price))} HBAR {isAuction ? "(reserve)" : ""}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <CalendarIcon className="h-4 w-4 text-base-content/60" />
            <span>Starts: {formatDate(effectiveStartDate)}</span>
          </div>
        </div>
